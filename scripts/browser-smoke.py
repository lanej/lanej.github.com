...
def main():
    ...
    parser.add_argument('--selection', help='Page selection JSON generated by viewrule.py select.')
    ...
    routes = ROUTES
    if args.selection:
        selected = set(json.loads(Path(args.selection).read_text())['routes'])
        routes = tuple((route, label) for route, label in ROUTES if route in selected)

    if not routes:
        report = {'status': 'passed', 'checks': [], 'reason': 'No affected browser routes'}
        (out / 'report.json').write_text(json.dumps(report, indent=2))
        print(json.dumps({'status': 'skipped', 'browser_checks': 0}))
        return
