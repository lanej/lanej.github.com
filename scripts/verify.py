"""Compare production bytes with the tested build, then inspect real browsers."""
import argparse
import hashlib
import io
import json
...
def main():
    ...
    if has_writing:
        ...
        for path in sorted(manifest['files']):
            if path.startswith('/writing/') and path.endswith('/index.html') and path != '/writing/index.html':
                routes.append((path.removesuffix('index.html'), path.strip('/').replace('/', '-')))
    with sync_playwright() as pw:
        ...
        for attempt in range(30 if live else 1):
            try:
                ...
                if result.ok and result.json().get('revision') == manifest['revision']:
                    break
            except Exception:
                pass
            if not live:
                raise AssertionError('Local manifest unavailable')
            time.sleep(10)
        else:
            raise AssertionError('Expected revision not visible on lanej.io within five minutes')
        for path, digest in manifest['files'].items():
            for attempt in range(18 if live else 1):
                response = request.get(urljoin(args.url, path), timeout=20000)
                if (response.ok or (path == '/404.html' and response.status == 404)) and hashlib.sha256(response.body()).hexdigest() == digest:
                    break
                if live:
                    time.sleep(10)
            else:
                raise AssertionError(f'{path}: HTTP {response.status} or bytes differ from tested build')
            if path.lower().endswith(('.jpg', '.jpeg', '.webp', '.png', '.gif')):
        ...
        for engine in args.engines.split(','):
            ...
            for width in widths:
                ...
                for route, label in routes:
                    ...
                    assert response and (response.ok or (route == '/404.html' and response.status == 404)), f'{engine} {route}: failed navigation'
                    page.locator('img').evaluate_all('(imgs)=>Promise.all(imgs.map(i=>i.decode()))')
                    assert page.locator('meta[name="site-revision"]').get_attribute('content') == manifest['revision'], f'{route}: wrong live revision'
                    metrics = page.evaluate('''()=>({width:innerWidth,scrollWidth:document.documentElement.scrollWidth,
                      nav:[...document.querySelectorAll('nav[aria-label="Primary navigation"] a')].map(a=>({text:a.textContent,height:a.getBoundingClientRect().height})),
                      images:[...document.images].map(i=>({src:i.currentSrc,naturalWidth:i.naturalWidth,width:i.getBoundingClientRect().width,height:i.getBoundingClientRect().height,portrait:i.hasAttribute('data-portrait'),radius:getComputedStyle(i).borderRadius})),
                      avatarCount:document.querySelectorAll('.header-portrait').length,
                    ...
                    assert metrics['avatarCount'] == (0 if route == '/' else 1), 'Interior-page avatar missing or duplicated'
                    assert not page.locator('.portrait-aside').count(), 'Large interior portrait remains'
                    for image in metrics['images']:
                        if not image['portrait']:
                            continue
                        assert abs(image['width'] - image['height']) < 2, 'Distorted headshot'
                        assert image['radius'] == '50%', 'Headshot is not circular'
                        ...
                        if route != '/':
                            assert image['width'] <= 56, 'Interior portrait is not compact'
                    if route == '/':
                        assert metrics['workTop'] < (1000 if width < 700 else 740), 'Hero pushes work too far down'
                    nav_writing = page.locator('nav[aria-label="Primary navigation"] a[href="/writing/"]')
                    assert bool(nav_writing.count()) == has_writing, 'Writing navigation does not reflect published content'
                    if route.startswith('/writing/') and route != '/writing/':
                        assert page.locator('meta[property="og:type"]').get_attribute('content') == 'article'
                        assert page.locator('.article-meta time').count() >= 1
                        assert any(item.get('@type') == 'BlogPosting' for item in [json.loads(s) for s in page.locator('script[type="application/ld+json"]').all_text_contents()])
                        if page.locator('.article-toc summary').count():
                            page.locator('.article-toc summary').click()
                    page.evaluate('document.activeElement?.blur()')
                    # A fresh navigation resets sequential focus after interacting with a TOC.
