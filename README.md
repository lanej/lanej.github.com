...
- `assets/icons/` and `static/favicon.ico`: circular portrait icons for tabs and an opaque iOS home-screen tile. Every Hugo page declares the fingerprinted PNGs; `static/apple-touch-icon.png` publishes the same dark tile at the domain root for Safari discovery, including standalone project pages without their own icon metadata. Rebuild both touch-icon copies from the approved photograph with `bash scripts/build-icons.sh` (ImageMagick 6 or 7). The build verifies the root fallback, its opaque dark-green corners, and icon declarations on every page. Standalone project repositories should still declare their own icons explicitly.
